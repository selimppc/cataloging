...
class Etsb_Cateditor_Adminhtml_CateditorController extends Mage_Adminhtml_Controller_Action
{

    protected function _initAction() {
        $this->loadLayout()
            ->_setActiveMenu('cateditor/items')
    ...
    public function editAction() {
        ...
        $model  = Mage::getModel('cateditor/cateditor')->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            Mage::register('cateditor_data', $model);
            ...
            $this->loadLayout();
            $this->_setActiveMenu('cateditor/items');
            ...
            $this->_addContent($this->getLayout()->createBlock('cateditor/adminhtml_cateditor_edit'))
                ->_addLeft($this->getLayout()->createBlock('cateditor/adminhtml_cateditor_edit_tabs'));

            $this->renderLayout();
        } else {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('cateditor')->__('Item does not exist'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            ...
            $up_model  = Mage::getModel('cateditor/cateditor')->load($up_id);


            $model = Mage::getModel('cateditor/cateditor');
            $model->setData($data)
                ->setFlagcatalog(1)
            ...
            try {
                ...
                $model->save();
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('cateditor')->__('Banner Image was successfully saved'));
        ...
        if(!is_array($platformIds)) {
            ...
            try {
                foreach ($platformIds as $platformId) {
                    $platform = Mage::getModel('cateditor/cateditor')->load($platformId);
                    $platform->delete();
                }
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($platformIds)) {
            ...
            try {
                foreach ($platformIds as $platformId) {
                    $platform = Mage::getSingleton('cateditor/cateditor')
                        ->load($platformId)
    ...
    {
        $fileName   = 'cateditor.csv';
        $content    = $this->getLayout()->createBlock('cateditor/adminhtml_cateditor_grid')
            ->getCsv();
